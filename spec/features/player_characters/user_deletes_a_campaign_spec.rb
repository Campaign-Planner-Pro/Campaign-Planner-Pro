...
RSpec.describe 'User deletes a player character', type: :feature do
  before(:each) do
  ...
  describe 'a user goes to a campaign' do
    scenario 'within a campaign a user deletes a player character' do
      visit campaign_path(@campaign1)

      click_link "Delete #{@pc1.name}"

      expect(current_path).to eq(campaign_path(@campaign1))
      expect(page).to have_content('Player Character was successfully deleted.')
