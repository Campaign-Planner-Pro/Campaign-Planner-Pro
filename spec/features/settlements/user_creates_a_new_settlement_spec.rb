...
RSpec.describe 'User creates a new settlement', type: :feature do
  before(:each) do
  ...
  describe 'a user creates a campaign' do
    scenario 'within a region a user creates a new settlement' do
      visit campaign_path(@campaign)

      click_link 'Create new Settlement in The Shire'

      expect(current_path).to eq(new_campaign_region_settlement_path(@campaign, @region))

      fill_in 'Name', with: 'Hobbiton'
      select  'Village', from: 'Size'
